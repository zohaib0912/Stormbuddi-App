...
const UploadInspectionReportModal = ({ 
  ...
  // Animate modal in and out from right side
  useEffect(() => {
    if (visible) {
      // Reset animation to start position (off-screen to the right)
  ...
  const chooseFromGallery = async () => {
    ...
    try {
      ...
      // Process each image with custom name prompt
  ...
  const chooseDocument = async () => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        // Process each document with custom name prompt
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add all files to FormData with custom file names
      formData.files.forEach((fileItem, index) => {
        formDataToSend.append('files[]', {
          uri: fileItem.fileUri,
          type: fileItem.fileType,
          name: fileItem.fileName, // This is the custom name
        });
        // Also send custom_file_name for each file (backend might expect this)
        if (fileItem.fileName) {
          formDataToSend.append('custom_file_names[]', fileItem.fileName);
        }
      ...
      if (data.success || response.status === 200) {
        ...
        // Prepare response data with custom_file_name for onSubmit callback
        // Handle both single file and array responses
        let responseFiles = [];
        if (Array.isArray(data.data)) {
          responseFiles = data.data;
        } else if (data.data) {
          responseFiles = [data.data];
        }
        
        // Extract custom_file_name from each file in response
        const responseData = {
          ...formData,
          uploadedFiles: responseFiles,
          // Map custom_file_name for easy access
          customFileNames: responseFiles.map(file => 
